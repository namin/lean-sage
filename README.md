...
| `Frame.lean` | partial (~1500 LOC, 6 sorries) | tower-aware `WFCtxT` (13 fields), `TowerCross` (12 fields), `FrameStmtT`, `frame_tower` defined. `acceptAllPolicy_respects_bisimT`, `HeapValid_append_closed`, `primPairs_atoms`, `buildBindings_extras_closed`, `freshLevelEnv_extras_closed`, `freshLevelEnv_HeapValid_preserves`, `materializeStep_HeapValid_preserves`, `materializeStep_iter_HeapValid_preserves`, **`materialize_HeapValid_preserves` all PROVED**. **Proved**: zero (all 4); eval cases `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect `.builtinBaseApply`/`.prim`/`.closure`. **`.em` mostly complete** — only 1 inner sorry: `h_env_mat` (cross-side env self-bisim at materialized level). **Case-level sorries**: `.set`, full `applyVia` clause, 2 materialize-preservation lemma bodies (`level_envs_valid` and `policies_resp`), `eval_preserves_envAt` body, and `h_env_mat` |

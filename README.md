...
| `Tower.lean` | partial (478 LOC, 2 sorries) | LevelState, TowerState, materialization, accessors, `RunState := TowerState` shim, foundational lemmas: `setPolicyAt`/`updateHeap`/`alloc`/`materialize` preservation facts; `materialize_envAt?_preserves`, `materialize_heap_grows`, `materialize_cross_side_some_iff`, `freshLevelEnv_heap_length`, `buildBindings_foldl_env_eq_of_len_eq`, `buildBindings_foldl_length` all proved. **Sorry'd**: `freshLevelEnv_env_eq` (Lean's `whnf` times out on the 13-element foldl normalization — fixable via `opaque`/`Decidable.decide` workaround) and `materialize_cross_side_envs_eq` (depends on `freshLevelEnv_env_eq` + Nat.fold induction). Used by `.em` / `applyVia` in Frame |
...
| `Frame.lean` | partial (1134 LOC, 4 sorries) | tower-aware `WFCtxT`, `TowerCross`, `FrameStmtT`, `frame_tower` defined. **Proved**: zero (all 4 functions); eval `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect non-applicable cases + `.builtinBaseApply` + `.prim` + **`.closure` (structurally complete)**. **Case-level sorries**: eval `.set`, `.em`, full `applyVia` clause + the auxiliary `eval_preserves_envAt` lemma body (used by `.closure`). `.em` and `applyVia` need a `Tower.materialize`-bisim-preservation lemma (cross-level lift). `.set` is the operational governance case (uses `ValVis_aux_update`). The `(TowerState, Env)` ↔ `(Heap, Env)` foldl mismatch was resolved by moving `allocStep` to `Tower.lean` and refactoring `Eval.lean`'s closure case to use it directly |

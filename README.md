...
| `Frame.lean` | partial (~1450 LOC, 7 sorries) | tower-aware `WFCtxT` (13 fields), `TowerCross` (12 fields), `FrameStmtT`, `frame_tower` defined. `acceptAllPolicy_respects_bisimT` proved. 3 single-side materialize-preservation lemmas exposed (sorry'd bodies). **Proved**: zero (all 4); eval cases `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect `.builtinBaseApply`/`.prim`/`.closure` (structurally complete). **`.em` mostly complete** — outer WFCtxT, HeapEvolution, and EnvVis all constructed (`h_he_mat` closed via the now-proved `materialize_heap_extends`). Only 1 inner sorry remains: `h_env_mat` (cross-side env self-bisim at materialized level — for the first-install case where level+1 is fresh, the bindings are atoms and self-bisim trivially). **Case-level sorries**: `.set`, full `applyVia` clause, the 3 materialize-preservation lemma bodies, `eval_preserves_envAt` body, and `h_env_mat` |

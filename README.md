...
| `Frame.lean` | partial (1090 LOC, 5 sorry sites — 3 cases + 2 inner) | tower-aware `WFCtxT`, `TowerCross`, `FrameStmtT`, `frame_tower` defined. **Proved**: zero (all 4 functions); eval `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect non-applicable cases + `.builtinBaseApply` + `.prim`. **Closure structurally proved** with two inner sorries (`levels_mono_a/b` — needs an `eval_preserves_envAt` lemma showing eval doesn't lose materialized levels). **Sorry'd at the case level**: eval `.set`, `.em`, full `applyVia` clause. `.em` and `applyVia` need a `Tower.materialize`-bisim-preservation lemma (cross-level lift). `.set` is the operational governance case (uses `ValVis_aux_update`). The `(TowerState, Env)` ↔ `(Heap, Env)` foldl mismatch was resolved by moving `allocStep` to `Tower.lean` and refactoring `Eval.lean`'s closure case to use it directly |

...
| `Frame.lean` | partial (976 LOC, 4 sorries) | tower-aware `WFCtxT`, `TowerCross`, `FrameStmtT`, `frame_tower` defined. **Proved**: zero (all 4 functions); eval `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect non-applicable cases + `.builtinBaseApply` + `.prim`. **Sorry'd**: eval `.set`, `.em`, full `applyVia` clause, applyDirect `.closure`. The remaining four are the genuinely hard cases — `.em` and `applyVia` need a `Tower.materialize`-bisim-preservation lemma (cross-level lift); `.set` is the operational governance case (uses `ValVis_aux_update`); `.closure` needs `alloc_chain_bisim` adapted to `TowerState` |

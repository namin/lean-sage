...
| `Frame.lean` | partial (~1620 LOC, 5 sorries) | tower-aware `WFCtxT` (13 fields), `TowerCross` (12 fields), `FrameStmtT`, `frame_tower` defined. `acceptAllPolicy_respects_bisimT` proved. **Two of three single-side materialize-preservation lemmas now fully proved**: `materialize_HeapValid_preserves` (via `HeapValid_append_closed`/`primPairs_atoms`/`buildBindings_extras_closed`/`freshLevelEnv_extras_closed` chain) and `materialize_level_envs_valid_preserves` (via `buildBindings_foldl_env_lookups_in_range`/`freshLevelEnv_env_valid` chain). Only `materialize_policies_resp_preserves` remains sorry'd. **Proved**: zero (all 4); eval cases `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect `.builtinBaseApply`/`.prim`/`.closure`. **`.em` mostly complete** — only 1 inner sorry (`h_env_mat`). **Case-level sorries**: `.set`, full `applyVia` clause, 1 materialize-preservation lemma body (`policies_resp`), `eval_preserves_envAt` body, and `h_env_mat` |

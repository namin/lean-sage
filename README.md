...
| `Frame.lean` | partial (~1300 LOC, 4 sorries) | tower-aware `WFCtxT` (13 fields including `level_envs_eq`, `policies_eq`, `policies_resp_all`), `TowerCross` (12 fields, mirroring), `FrameStmtT`, `frame_tower` defined. **Proved**: zero (all 4 functions); eval `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect non-applicable cases + `.builtinBaseApply` + `.prim` + `.closure` (structurally complete via `eval_preserves_envAt`). **Case-level sorries**: eval `.set`, `.em`, full `applyVia` clause + the auxiliary `eval_preserves_envAt` lemma body. `.em` now has all Tower lemmas it needs (`materialize_cross_side_*` are proved); the remaining gap is sub-lemmas about materialize preserving HeapValid and policy validity (~50-100 LOC), then the case body itself (~100 LOC). `applyVia` follows the same pattern. `.set` is the operational governance case (uses `ValVis_aux_update`) |

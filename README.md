...
| `Frame.lean` | partial (~1450 LOC, 6 case sorries + 2 inner) | tower-aware `WFCtxT` (13 fields), `TowerCross` (12 fields), `FrameStmtT`, `frame_tower` defined. `acceptAllPolicy_respects_bisimT` proved. 3 single-side materialize-preservation lemmas exposed (sorry'd bodies). **Proved**: zero (all 4); eval cases `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`/`.letE`/`.installPolicy`; full `evalList`; applyDirect `.builtinBaseApply`/`.prim`/`.closure` (structurally complete). **`.em` structurally complete** — outer WFCtxT and EnvVis are now constructed (no longer sorry'd!) by composing `materialize_HeapEvolution` (sub-sorry) with the IH output. Only 2 inner sorries remain in `.em`: `h_env_mat` (env self-bisim cross-side at materialized level) and `h_he_mat` (HeapEvolution-from-materialize, needs `materialize_heap_extends` lemma). **Case-level sorries**: `.set`, full `applyVia` clause, the 3 materialize-preservation lemma bodies, and `eval_preserves_envAt` body |

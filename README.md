...
| `Frame.lean` | partial (727 LOC, 6 sorries) | tower-aware `WFCtxT`, `TowerCross`, `FrameStmtT`, `frame_tower` defined. **Proved**: zero (all 4 functions); eval `.num`/`.bool`/`.quote`/`.var`/`.lam`/`.ifte`/`.seq`/`.app`/`.primApp`; full `evalList`; applyDirect non-applicable cases + `.builtinBaseApply` + `.prim`. **Sorry'd**: eval `.set`/`.em`/`.letE`/`.installPolicy`, full `applyVia` clause, applyDirect `.closure`. The `TowerCross` structure bundles cross-side WFCtx invariants at output (heap-len-eq, policy-eq-at, levels-mono, HeapValid, level-envs-valid, policy-resp) so that `applyVia`/`applyDirect` can hand back enough information for the caller to reconstruct a full `WFCtxT` |
